...
lo.inState = function(state, options) {
  var options = options || {}
  var states = this.states;
  var latest = states[states.length-1]

  if(!options.exact) {
    if(latest === state || latest.indexOf(state + '.') === 0 ) {
      return true
    } else {
      return false
    }
...
var rules = {
  ...
  JST_EXPR_OPEN: ['{BEGIN}',function(all, one){
    if(all === this.markStart){
      ...
      if(this.inState('JST.OPEN') || this.inState('JST.EXPR_OPEN') || this.marks){
        this.marks++
        return { type: this.markStart, value: this.markStart };
      }else{
        this.enter('JST.EXPR_OPEN')
      }
    }
    return {
      type: 'EXPR_OPEN',
